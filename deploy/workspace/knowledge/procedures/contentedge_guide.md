...
### Date and retention options (PDF p.434+)

- Delete equal-to-or-older than a timestamp (`YYYYMMDDHHMMSS`):
  - `Use -t YYYYMMDDHHMMSS (default behavior is delete all ingestions equal to or older than that timestamp).`

- Delete strictly before `2026-01-01`:
  - `Use -t 20251231235959 (because -t is inclusive).`

- Delete older than N days:
  - `Use -d <days> (for example: -d 365).`

- Delete a report version timestamp range:
  - `Use -TRN <first_timestamp> -TRO <last_timestamp>.`

### Content class scope

- Delete one content class:
  - `Use -r <ContentClassID> (for example: -r AC001).`

- Use wildcard content class IDs:
  - `Use trailing * in -r and add -Y w (for wildcard support).`

- Multiple content classes:
  - `Run one adelete command per -r class ID (or use wildcard carefully).`

### Processing options (`-Y`)

- Common deletion set:
  - `-Y ALL` (equivalent to `iavfdpe`; excludes `u`, `w`, `c`, `n`, `x`).

- Include wildcard support:
  - `-Y w`.
